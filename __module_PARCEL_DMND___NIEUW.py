...
def actually_run_module(args):
    ...
    try:
        ...
        # Add the B2C parcels to the zones dataframe
        zones['parcels_B2C'] = (demandPerZone['parcels'] / 60)

        # ---------------------------------------------------------------------
        # 7) Calibrate no. of B2C parcels to get the target number of 
        # parcels in the study area
        

        zones['woningen']   = segs['1: woningen'].copy()
        zones['inwoners']   = segs['2: inwoners'].copy()
        zones['arbeidspl']  = segs['9: arbeidspl_totaal'].copy()

        # parcels_target = varDict['PARCELS_TARGET']
        # parcels_target_inw = zones['inwoners'].sum() * 0.0568
        # parcels_target_hh2  = zones['woningen'].sum() * 0.1129

        parcels_target_hh  = zones['woningen'].sum() * varDict['PARCELS_PER_HH']

        #Number of B2C parcels calculated with demand model to be scaled up or down to 
        #match target for total study area; 
        #target is calculated as input factor PARCELS_PER_HH * no. of HH
    
        parcels_b2c = zones['parcels_B2C'].sum()
        zones['parcels_B2C_cal'] = zones['parcels_B2C'] * parcels_target_hh / parcels_b2c 
          
        print('\t\tApply calibration factor', np.round(parcels_target_hh / parcels_b2c, 3), 'to B2C parcels...')
        log_file.write('\t\tApply calibration factor', np.round(parcels_target_hh / parcels_b2c, 3), 'to B2C parcels...')
        
        
        zones['parcels_total'] = zones['parcels_B2C_cal'] + zones['parcels_B2B']   
        
        
        
        
        # zones_copy_old = zones_copy.copy()
        ...
        zones_copy['parcels_pppd']  = zones_copy['parcels_B2C'] / zones_copy['inwoners']
        zones_copy['parcels_pppd'] = zones_copy['parcels_pppd'].fillna(0)
        
        


        plt.hist(zones_copy['parcels_pppd'][zones_copy['parcels_pppd']>0], bins=25)
        plt.show()

        min1= zones_copy['parcels_pppd'][zones_copy['parcels_pppd']>0].min()
        
        min_03= zones_copy[zones_copy['parcels_pppd']<0.03]
        min_3= min_03[min_03['parcels_pppd']>0]
        ...
        ### EN
        # ---------------------------------------------------------------------
        # 8) Spread over couriers based on market shares
        for cep in cepList:
...
if __name__ == '__main__':
    ...
    varDict['PARCELS_PER_HH']	= 0.1129
    varDict['PARCELS_PER_EMPL'] = 0.0229
    varDict['PARCELS_TARGET']   = 300000
